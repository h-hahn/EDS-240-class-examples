...
fracking_clean <- fracking %>% 
  ...
  # clean column names ----
  janitor::clean_names() %>% # update column names to lower snake case "_"

  # clean up dates ----
  mutate(job_start_date = str_remove(job_start_date, " AM")) %>% # remove 'AM' from the job_start_date column
  mutate(datetime_start = mdy_hms(job_start_date)) %>% # convert the job_start_date from a string to datetime as a new column 
  mutate(year = year(datetime_start)) %>% # create 'year' col from date

  # select relevant cols ----
  select(datetime_start, year, state_name, county_name, well_name, total_base_water_volume) %>% # specify names of columns to keep
  
  # filter out non-state names ----
  filter(!state_name %in% c("Beaver", "Beckham", "Harper", "Hemphill", "Midland", "Red River", "Roosevelt", "Rusk", "State", "WARD")) %>% # specify names of non-state names to remove
  ...
  mutate(sn = str_to_title(sn)) %>% # change strings in sn column to title case

  # fix misspelled state names ---- 
  mutate(sn = case_when( sn == "Colordao" ~ "Colorado", 
                         ...
                         sn == "Texasa" ~ "Texas", sn == "Texs" ~ "Texas", 
                         sn == "West Viginia" ~ "West Virginia", 
                         sn == "Wyominng" ~ "Wyoming", 
                         TRUE ~ sn # copy over rest of state names from as-is 
                         )) %>% # identify mispellings and overwrite with correct names
  ...
  mutate(sn = str_to_upper(sn)) %>% # convert strings within sn column to all uppercase
  ...
  mutate(sn = str_to_title(sn)) %>% # convert the uppercase strings in sn back to title case
  ...
  mutate(state_name = as.factor(state_name), state_abb = as.factor(state_abb)) %>% 
